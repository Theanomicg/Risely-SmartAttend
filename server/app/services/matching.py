...
async def find_best_student_match(session: AsyncSession, embedding: list[float]) -> MatchResult:
    sql = text(
        """
        SELECT
            s.uid,
    ...
    result = await session.execute(sql, {"embedding": to_pgvector_literal(embedding)})
